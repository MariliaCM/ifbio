...
if (!empty($_GET['search'])) {
  ...
  $sql = "SELECT iddaninha, nomedominio, nomereino, nomefilo, nomeclasse, nomeordem, nomefamilia, nomegenero, nomeespecie, nomeflora, problema from dominio inner join reino on iddominio=fkreino inner join filo on idreino=fkfilo inner join classe on idfilo=fkclasse inner join ordem on idclasse=fkordem inner join familia on idordem=fkfamilia inner join genero on idfamilia=fkgenero inner join especie on idgenero=fkespecie inner join flora on idespecie=fkflora inner join daninha on idflora=fkdaninha ORDER BY iddaninha ASC;";
}
          ...
          <tbody>
            ...
            while ($user_data = mysqli_fetch_assoc($result)) {
              ...
              if (!empty($_SESSION['id'])) {
                echo "<td>
                        <a class='btn btn-sm btn-primary' href='ervas_edit.php?iddaninha=$user_data[iddaninha]' title='Editar'>
                            <svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='bi bi-pencil' viewBox='0 0 16 16'>
                                <path d='M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l6.5-6.5zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325z'/>
                            </svg>
                            </a> 
                            <a class='btn btn-sm btn-danger' href='ervas_del.php?iddaninha=$user_data[iddaninha]' title='Deletar'>
...
<script>
  ...
  function searchData() {
    window.location = 'ervas_pesquisar.php?search=' + search.value;
  }
</script>